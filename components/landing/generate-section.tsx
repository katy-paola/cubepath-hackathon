...
export function GenerateSection({ className, ...props }: GenerateSectionProps) {
  ...
  function updateField<Key extends keyof RoutineFormValues>(key: Key, value: RoutineFormValues[Key]) {
  ...
  async function handleGenerateRoutine(event?: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await consumeRoutineStream(response, async (evt, data) => {
        ...
        if (evt === "progress") {
          setStreamStatus(`Generando día ${data.generatedDays}/${data.totalDays}...`);
          return;
        }
